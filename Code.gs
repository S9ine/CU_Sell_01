...
return data.map(row => ({ id: row[0], name: row[1], tel: row[3] }));
  } catch (e) { console.error("getCustomers Error: " + e.message);
return [];
  }
}
...
return { success: true, message: "ลบข้อมูลลูกค้าสำเร็จ" };
  } catch (e) { console.error("deleteCustomer Error: " + e.message);
return { success: false, message: e.message };
  }
}
...
if (!stockCheck.isValid) {
    ...
    if (salesSheet.getLastRow() === 0) {
      salesSheet.appendRow(['เลขที่เอกสาร', 'วันที่ขาย', 'รหัสลูกค้า', 'ชื่อลูกค้า', 'เบอร์ติดต่อ', 'รหัสพนักงาน', 'พนักงานขาย', 'ยอดรวม', 'ส่วนลด', 'ยอดสุทธิ', 'แผงไข่ส่ง', 'แผงไข่รับ', 'ชื่อสินค้า', 'จำนวน', 'หน่วย', 'ราคาต่อหน่วยย่อย', 'ราคารวม', 'จำนวนหน่วยย่อย', 'ผู้บันทึก', 'เวลาที่บันทึก']);
}

    const recordsToSave = formData.items.map((item, index) => {
      const commonData = index === 0 ? [ invoiceId, timestamp, formData.customerId, formData.customerName, "'" + formData.customerTel, formData.employeeId, formData.salesperson, formData.subtotal, formData.discount, formData.grandTotal, formData.traysSent, formData.traysReceived ] : Array(12).fill('');
...
function getEmployeeList() {
    // [FIX] Changed cache key to 'employeeList_v2' to avoid conflicts with old cached data format.
    return getCachedData_('employeeList_v2', () => {
        try {
            const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.webAppInfo.configSheet);
            if (!sheet || sheet.getLastRow() < 3) return [];
            const values = sheet.getRange(`M3:N${sheet.getLastRow()}`).getValues();
            return values.map(row => ({ id: row[0], name: row[1] })).filter(emp => emp.id && emp.name);
        } catch (e) 
...
function clearServerCache() {
  try {
    // [FIX] Added old 'employeeList' and new 'employeeList_v2' keys to ensure all versions are cleared.
    CacheService.getScriptCache().removeAll(['productList', 'employeeList', 'employeeList_v2']);
